...
import { pickerFilter, textFilter, idMatches, focusSubtitle } from "./filters";
...
async function salesReport(scope, from, to, options = {}) {
    ...
    // picker is offered there. Both lists are built from this window's own data.
    ...
    // the product filter by containing that product.
    const ordersWithProduct = productId
        ? new Set(allItems.filter((i) => i.productId === productId).map((i) => i.orderId))
        : null;
    const orders = allOrders.filter((o) =>
        (!storeId || o.storeId === storeId) &&
        (!ordersWithProduct || ordersWithProduct.has(o.id))
    );
    const keptOrderIds = new Set(orders.map((o) => o.id));
    const items = allItems.filter((i) => keptOrderIds.has(i.orderId) && (!productId || i.productId === productId));

    const paid = orders.filter((o) => o.paymentStatus === 'PAID');
    const pending = orders.filter((o) => o.paymentStatus === 'PENDING');
    // With a product filter the report is about that product, so revenue is read
    // off its own lines rather than off the whole orders it was bought in.
    ...
    // Revenue attributed to a single order. Under a product filter only that
    // product's lines count, so every breakdown below adds up to the same total
    // the KPI reports instead of over-counting the whole basket it was bought in.
    ...
    const orderRevenue = (o) => (productId ? (lineValueByOrder.get(o.id) || 0) : Number(o.total || 0));
    ...
    const ledger = await salesLedger(paid, items, storeMap, allItems);

    return finalize('sales', scope, from, to, {
        subtitle,
        filters,
        kpis: [
            { label: productId ? 'Product revenue (paid)' : 'Revenue (paid)', value: round(revenue), format: 'currency' },
            ...
            { label: productId ? 'Avg per order' : 'Avg order value', value: round(aov), format: 'currency' },
...
async function categoryPathsByName() {
...
async function salesLedger(paidOrders, items, storeMap, allItems) {
    ...
    const [products, categories] = await Promise.all([
        productIds.length
            ? prisma.product.findMany({ where: { id: { in: productIds } }, select: { id: true, name: true, category: true, price: true, warehouseQuantity: true } })
            : Promise.resolve([]),
        categoryPathsByName(),
    ]);
