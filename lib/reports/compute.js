...
async function salesReport(scope, from, to) {
    ...
    const ledger = await salesLedger(paid, items);
...
async function salesLedger(paidOrders, items) {
    ...
    const productIds = [...new Set(items.map((i) => i.productId))];
    const storeIds = [...new Set(paidOrders.map((o) => o.storeId).filter(Boolean))];
    ...
    const [products, stores, categories] = await Promise.all([
        productIds.length
            ...
            : Promise.resolve([]),
        storeIds.length
            ? prisma.store.findMany({ where: { id: { in: storeIds } }, select: { id: true, name: true, sellerModel: true } })
            : Promise.resolve([]),
        categoryPathsByName(),
    ]);
    ...
    const transport = allocateTransport(paidOrders, itemsByOrder);
...
async function deliveriesReport(scope, from, to) {
    ...
    // Corridor lookup for the ledger. A shipment booked inside this window can sit
    // on a corridor that runs outside it, so hydrate whatever the range missed.
    const corridorMap = new Map(corridors.map((c) => [c.id, c]));
    const missingCorridorIds = [...new Set(orders.map((o) => o.corridorId).filter((id) => id && !corridorMap.has(id)))];
    ...
    const ledger = await deliveryLedger(orders, corridorMap, riderMap);
...
async function deliveryLedger(orders, corridorMap, riderMap) {
    ...
    const [addressMap, hubMap, storeMap, userMap, itemRows, config, ranges] = await Promise.all([
        byId('address', orders.map((o) => o.addressId), { name: true, village: true, cell: true, sector: true, district: true, city: true }),
        byId('deliveryHub', orders.map((o) => o.dropHubId), { name: true, sector: true }),
