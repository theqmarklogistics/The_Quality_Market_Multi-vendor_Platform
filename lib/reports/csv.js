...
export function reportToCsv(report) {
    ...
    for (const section of report.sections || []) {
        ...
        if (section.kind === 'timeseries') {
            ...
            // Totals footer: the column's total where it has one, the "Total"
            // label in the first column, blanks elsewhere.
            if (section.totals && (section.rows || []).length) {
                out.push(line(cols.map((c, i) => {
                    const total = section.totals[c.key];
                    if (total !== undefined && total !== null) return rawValue(total, c.format);
                    return i === 0 ? (section.totalsLabel || 'Total') : '';
                })));
            }
