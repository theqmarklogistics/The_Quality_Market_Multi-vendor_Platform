...
export async function runPoolBatching() {
    ...
    for (const [sector, groupOrders] of groups.entries()) {
        // Order stops nearest-first by hub→drop ROAD distance (one matrix call,
        // persisted per order below), then split into routes of at most
        // MAX_STOPS_PER_CORRIDOR stops (one corridor/rider each). Each stop
        ...
        const ordered = orderStopsByDistance(points, dropKm);
        ...
        for (let r = 0; r < routes.length; r++) {
            ...
            // reusing the persisted hub→drop road distances where known.
            const dropDistances = routeStops.map((s) => s.deliveryDistanceKm);
            const distOpts = dropDistances.every((d) => d != null)
                ? { dropDistances, routeKm: await fullRouteKm(routeStops, KIGALI_HUB) }
                : {};
            const { routePrice, shares } = computeRouteShares(routeStops, config, weightRanges, KIGALI_HUB, distOpts);
