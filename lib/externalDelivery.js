...
export async function quoteExternalDeliveryFee(opts = {}) {
    ...
    if (distanceKm != null && distanceKm > 0 && greaterWeightKg > 0) {
        ...
        const q = computeShippingFee({ greaterWeightKg, volumetricKg: volKg, actualKg, distanceKm, ranges, config });
        if (q.needsReview) {
            return { ...base, fee: null, needsReview: true, chargeableWeightUsed: null, basis: "needs_review" };
        }
...
export async function quotePooledCartFee({ items = [], lat = null, lng = null, sector = null, originLat = null, originLng = null } = {}) {
    ...
    // Same precedence as quoteExternalDeliveryFee: corridor rate → formula → flat.
