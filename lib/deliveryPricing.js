...
// Billing rounds the chargeable weight UP to the nearest 0.5 kg (3.2 → 3.5).
...
// Weight the customer is billed for: chargeable weight, rounded up to 0.5 kg.
export function billedWeightKg(actualKg, l, w, h, volumetricFactor = DEFAULTS.volumetricFactor) {
    return roundUpToHalfKg(chargeableWeightKg(actualKg, l, w, h, volumetricFactor));
}
 ...
 * Fee = max(floor, round(baseRate × billedKg × distanceKm × multiplier(distanceKm))),
 * where billedKg is the chargeable weight rounded UP to the nearest 0.5 kg.
 */
export function quoteStandaloneFee({ chargeableKg, distanceKm, config }) {
    const c = resolvePricingConfig(config);
    const kg = roundUpToHalfKg(Math.max(0, Number(chargeableKg) || 0));
...
export function computeRouteShares(orderedStops, config, origin = KIGALI_HUB, opts = {}) {
    ...
    // Per-drop load = billedKg × hub→drop distance (road distance when supplied).
    // Weight is rounded up to 0.5 kg exactly like the standalone quote, so a
    // single-stop corridor always prices identically to a solo booking.
    const loads = orderedStops.map((s, i) => {
        const kg = roundUpToHalfKg(Math.max(0, Number(s?.chargeableKg) || 0));
