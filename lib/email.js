...
export async function sendInvoiceEmail({ to, subject, pdfBuffer, orderId, filename }) {
    await resend.emails.send({
        ...
        attachments: [
            {
                filename: filename || `invoice-${orderId?.slice(0, 8) || 'order'}.pdf`,
                content: pdfBuffer,
            },
        ],
    });
...
// ─── Threaded admin ↔ customer email ─────────────────────────────────────────
// Sends a branded message whose Reply-To routes the customer's answer back into
// the thread (inbox+<token>@INBOUND_EMAIL_DOMAIN → /api/email/inbound webhook).
// Returns the Resend message id so replies can also be matched by References.

export async function sendThreadEmail({ to, subject, bodyHtml, replyTo }) {
    const payload = {
        from: FROM,
        to,
        subject,
        html: `
            <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; background: #fff;">
                ...
                <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
                <p style="color: #94a3b8; font-size: 12px;">
                    You can reply directly to this email — your reply reaches our support team.<br />
                    The Quality Market &mdash; Kigali, KN 82 St, Tropical plaza, C26<br />
                    support@thequalitymarket.com &nbsp;|&nbsp; +250 783 610 209
                </p>
            </div>
...
// ─── Delivery booked — tracking link (to the recipient) ──────────────────────
// Sent the moment a delivery is booked so the recipient gets a live tracking
// link. The link carries the public tracking token (?t=…), so the recipient can
// open it with no platform login — and share their exact drop-off location from
// the tracking page, which is what prices and routes the delivery.

export async function sendDeliveryTrackingEmail({ to, orderId, trackingToken, recipientName, senderName, packageDescription }) {
    if (!to || !orderId || !trackingToken) return;
    const trackUrl = `${APP_URL}/track/${orderId}?t=${trackingToken}`;
    const shortId = orderId?.slice(0, 12) || 'your delivery';
    const greeting = recipientName ? `Hi ${recipientName},` : 'Hello,';
    const fromLine = senderName
        ? `A delivery from <strong>${senderName}</strong> is on its way to you.`
        : 'A delivery is on its way to you.';

    await resend.emails.send({
        from: FROM,
        to,
        subject: `📦 A delivery is on its way to you — track it live`,
        html: `
            <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; background: #fff;">
                ${BRAND_HEADER}
                <p style="color: #16a34a; font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; margin: 0 0 6px;">Kigali Delivery</p>
                <h2 style="color: #1e293b; margin: 0 0 8px;">Your delivery is on the way!</h2>
                <p style="color: #64748b; margin-bottom: 16px;">
                    ${greeting}<br />
                    ${fromLine} You can follow it live and see your rider's location on the map from the link below.
                </p>
                ${packageDescription ? `
                <div style="background: #f8fafc; border-left: 4px solid #16a34a; padding: 12px 16px; border-radius: 0 8px 8px 0; margin-bottom: 20px;">
                    <p style="color: #334155; font-size: 14px; margin: 0;"><strong>Package:</strong> ${packageDescription}</p>
                </div>` : ''}
                <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 14px 16px; margin-bottom: 20px;">
                    <p style="color: #166534; font-size: 13px; margin: 0;">
                        Tip: open the link and <strong>share your exact location</strong> so your rider can reach you faster.
                    </p>
                </div>
                <div style="text-align: center; margin: 28px 0;">
                    <a href="${trackUrl}"
                ...
                <p style="color: #64748b; font-size: 13px;">
                    ...
                    <a href="${trackUrl}" style="color: #2563eb; word-break: break-all;">${trackUrl}</a>
                </p>
                <p style="color: #94a3b8; font-size: 12px; margin-top: 16px;">Reference: #${shortId}</p>
                <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
                <p style="color: #94a3b8; font-size: 12px;">
                    The Quality Market &mdash; Kigali, KN 82 St, Tropical plaza, C26<br />
                    support@thequalitymarket.com &nbsp;|&nbsp; +250 783 610 209
...
// ─── Delivery fee — manual review alert (to admin) ───────────────────────────
// Fired when a package's greater (actual vs volumetric) weight falls outside
// every configured DeliveryWeightRange, so the fee could not be auto-computed.
// Admin must contact the customer and set the fee manually.

export async function sendDeliveryReviewAlertEmail({ orderId, kind = 'order', customerName, customerPhone, actualKg, volumetricKg, greaterWeightKg, distanceKm, sector }) {
    ...
    const shortId = orderId?.slice(0, 12) || 'unknown';
    const adminUrl = `${APP_URL}/admin/orders`;
    const row = (label, value) =>
        `<tr><td style="padding: 4px 12px 4px 0; color: #64748b; font-size: 13px;">${label}</td><td style="padding: 4px 0; color: #0f172a; font-size: 13px; font-weight: 600;">${value}</td></tr>`;

    await resend.emails.send({
        from: FROM,
        to: recipients,
        subject: `⚠️ Delivery needs manual pricing — ${kind} #${shortId}`,
        html: `
            <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; background: #fff;">
                ${BRAND_HEADER}
                <p style="color: #d97706; font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; margin: 0 0 6px;">Action needed</p>
                <h2 style="color: #1e293b; margin: 0 0 8px;">This delivery could not be auto-priced</h2>
                <p style="color: #64748b; margin-bottom: 16px;">
                    The package weight falls outside every configured weight range, so the shipping
                    fee was not calculated. Please contact the customer to confirm the package and
                    set the delivery fee manually.
                </p>
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                    ${row('Reference', `#${shortId}`)}
                    ${customerName ? row('Customer', customerName) : ''}
                    ${customerPhone ? row('Phone', customerPhone) : ''}
                    ${sector ? row('Sector', sector) : ''}
                    ${row('Actual weight', `${actualKg ?? 0} kg`)}
                    ${row('Volumetric weight', `${volumetricKg ?? 0} kg`)}
                    ${row('Chargeable (greater)', `${greaterWeightKg ?? 0} kg`)}
                    ${distanceKm != null ? row('Distance', `${distanceKm} km`) : ''}
                </table>
                <div style="text-align: center; margin: 28px 0;">
                    <a href="${adminUrl}"
                       style="display: inline-block; background: #1e293b; color: #fff;
...
// ─── Shipping quotation request (imported products, to admin) ────────────────
// Fired when a shopper asks for a shipping quote on imported (non-local) products.
// Imported items are not auto-priced by the domestic delivery formula, so the
// customer requests a quote and admin follows up with a price.

export async function sendShippingQuoteRequestEmail({ customerName, customerEmail, customerPhone, items = [], deliveryArea, note, source = 'product' }) {
    const recipients = (process.env.ADMIN_EMAIL || '')
        .split(',')
        .map((e) => e.trim())
        .filter(Boolean);
    ...
    const itemRows = items
        .map((it) => `<li style="margin-bottom: 4px;">${it.name}${it.origin ? ` <span style="color:#2563eb;">(imported · ${it.origin})</span>` : ''}${it.quantity ? ` &times; ${it.quantity}` : ''}</li>`)
        .join('');

    await resend.emails.send({
        from: FROM,
        to: recipients,
        subject: `📦 Shipping quote requested — imported item${items.length === 1 ? '' : 's'}`,
        html: `
            <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; background: #fff;">
                ${BRAND_HEADER}
                <p style="color: #2563eb; font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; margin: 0 0 6px;">Shipping quote request</p>
                <h2 style="color: #1e293b; margin: 0 0 8px;">A customer requested a shipping quote</h2>
                <p style="color: #64748b; margin-bottom: 16px;">
                    The item(s) below are imported and are not auto-priced by the delivery formula.
                    Please prepare a shipping quote and follow up with the customer.
                </p>
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 16px;">
                    ${customerName ? row('Customer', customerName) : ''}
                    ${customerEmail ? row('Email', customerEmail) : ''}
                    ${customerPhone ? row('Phone', customerPhone) : ''}
                    ${deliveryArea ? row('Delivery area', deliveryArea) : ''}
                    ${row('Requested from', source === 'checkout' ? 'Checkout' : 'Product page')}
                </table>
                <p style="color: #64748b; font-size: 13px; font-weight: 600; margin: 0 0 6px;">Imported items</p>
                <ul style="color: #0f172a; font-size: 13px; padding-left: 18px; margin: 0 0 16px;">${itemRows}</ul>
                ${note ? `<p style="color: #64748b; font-size: 13px; margin-bottom: 16px;"><strong>Note:</strong> ${note}</p>` : ''}
                <div style="text-align: center; margin: 28px 0;">
                    <a href="${adminUrl}"
                       style="display: inline-block; background: #1e293b; color: #fff;
                              text-decoration: none; padding: 12px 32px; border-radius: 100px;
