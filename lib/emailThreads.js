...
// Resend outbound delivery-lifecycle events → the status we persist. Ranked so
// out-of-order webhook delivery never regresses a message (a late "sent" can't
// overwrite "delivered"); bounces/complaints rank highest and are the actionable
// ones. "email.received" is handled separately (inbound), not here.
const OUTBOUND_EVENT_STATUS = {
    "email.sent": "sent",
...
const STATUS_RANK = { sent: 1, delayed: 2, delivered: 3, opened: 4, clicked: 5, complained: 6, bounced: 7 };

/** The delivery status a Resend event maps to, or null if it isn't one we track. */
export function outboundStatusForEvent(type) {
    return OUTBOUND_EVENT_STATUS[type] || null;
}

/**
 * Record a Resend delivery-lifecycle event against the OUTBOUND message it
 * refers to (matched by the Resend email id). Only advances status forward, so
 * duplicate or out-of-order webhooks can't regress it. Returns
 * { message, changed } — message is null when nothing matched the email id.
 */
export async function recordOutboundStatus({ type, emailId, at }) {
    const status = OUTBOUND_EVENT_STATUS[type];
    if (!status || !emailId) return { message: null, changed: false };

    const msg = await prisma.emailMessage.findFirst({
        where: { providerMessageId: emailId, direction: "OUTBOUND" },
        select: { id: true, threadId: true, toEmail: true, deliveryStatus: true },
    });
    if (!msg) return { message: null, changed: false };

    // Stale/duplicate event — keep the later state we already have.
    if (msg.deliveryStatus === status) return { message: msg, changed: false };
    if ((STATUS_RANK[status] || 0) < (STATUS_RANK[msg.deliveryStatus] || 0)) return { message: msg, changed: false };

    const message = await prisma.emailMessage.update({
        where: { id: msg.id },
        data: { deliveryStatus: status, statusAt: at ? new Date(at) : new Date() },
    });
